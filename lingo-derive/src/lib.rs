...
pub fn derive_config(input: TokenStream) -> TokenStream {
    ...
    // 使用 proc-macro-crate 动态解析依赖方对 `lingo` 的重命名
    let crate_ident = match proc_macro_crate::crate_name("lingo") {
        Ok(proc_macro_crate::FoundCrate::Itself) => quote! { crate },
        Ok(proc_macro_crate::FoundCrate::Name(name)) => {
            let ident = syn::Ident::new(&name, proc_macro2::Span::call_site());
            quote! { #ident }
        }
        Err(_) => quote! { lingo }, // 回退：直接使用 lingo
    };
    ...
    let expanded = quote! {
        impl #name {
            ...
            pub fn load() -> Result<Self, Box<dyn std::error::Error>> {
                ...
                let app_meta = #crate_ident::LingoAppMeta { app_name, env_prefix, behavior_version: 1, max_parse_depth: 128 };

                // 解析候选配置文件路径（宽容处理目录缺失场景）
                let mut config_file_paths = match #crate_ident::resolve_config_files(&app_meta) {
                    Ok(v) => v,
                    Err(#crate_ident::LingoError::NoConfigFilesFoundInDir { .. }) |
                    Err(#crate_ident::LingoError::ConfigDirNotFound { .. }) => Vec::new(),
                ...
                for cfg in config_file_paths {
                    let provider = #crate_ident::providers::LingoFileProvider::from_path(&cfg.path, cfg.is_required, app_meta.max_parse_depth)?;
                    fig = fig.merge(provider);
                }
                if let Some(prefix) = app_meta.env_prefix.clone() {
                    let env_provider = #crate_ident::providers::LingoEnvProvider::with_prefix(prefix);
            ...
            pub fn load_with_args(args: Vec<String>) -> Result<Self, Box<dyn std::error::Error>> {
                ...
                let clap_matches = #crate_ident::Command::new(cmd_name)
                    ...
                    .map_err(|e| #crate_ident::LingoError::Internal(format!("Failed to parse CLI args: {}", e)))?;
            ...
            pub fn load_from_file<P: AsRef<std::path::Path>>(path: P) -> Result<Self, Box<dyn std::error::Error>> {
                ...
                let provider = #crate_ident::providers::LingoFileProvider::from_path(path, true, 128)?;
