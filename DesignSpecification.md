**【软件设计文档 - Lingo (超详细版 for LLM Code Generation - v4 完整无省略版)】**
...
5.  关键功能流程或算法描述
    ...
    5.5. 配置文件键名构造逻辑
6.  所需软件栈与依赖 (含版本)
    6.1. 工作区 `lingo/Cargo.toml`
    6.2. `core` Crate `lingo/core/Cargo.toml`
    6.3. `macros` Crate `lingo/macros/Cargo.toml`
7.  关键设计决策解释
8.  数据模型 (用户定义与内部元数据)
...
*   **项目名称：** Lingo (玲珑)
*   **目标：** `Lingo` 是一个强大且灵活的 Rust 配置库，旨在简化应用程序配置的管理。它允许开发者通过简单的派生宏和字段属性，将配置从多种来源（包括 TOML/JSON/INI 文件、环境变量和命令行参数）无缝加载到自定义的 Rust 结构体中。
*   **核心价值：** 提供类型安全、易于使用、高度可定制、开发者友好、安全且具有前向兼容性的配置解决方案。
*   **哲学定位 (新增)：**
    > `Lingo` 并非旨在取代 `figment` 或 `clap`，而是作为一个“固执己见” (opinionated) 的上层框架。它通过封装这两个库 80% 的常用功能，为绝大多数应用场景提供了一个“开箱即用”、“约定优于配置”的解决方案。其核心价值在于**极致简化**，而非**极致灵活**。当用户需要 `clap` 的完整子命令系统或 `figment` 的全部高级特性时，直接使用这两个库可能是更合适的选择。
*   **主要功能概述：**
    *   **多源配置加载：** 支持从 TOML、JSON 和 INI 格式的配置文件、环境变量以及命令行参数加载配置。
    *   **优先级覆盖：** 严格按照预定义优先级（系统文件 < 用户文件 < 指定文件 < 环境变量 < 命令行参数）合并配置，高优先级源覆盖低优先级源。
    *   **过程宏驱动：** 通过 `#[derive(Config)]` 和字段级 `#[lingo_opt(...)]` 属性，以及结构体级 `#[config(...)]` 属性（用于版本控制等元数据），简化配置结构体的定义和元数据指定。
    *   **与 `clap` 深度集成：** 自动处理命令行参数的解析，包括标准参数（如 `--version`, `--help`，由 `clap` 自动生成）和用户自定义参数，均通过 `#[lingo_opt(...)]` 属性驱动。
    ...
    *   **详细的错误报告：** 提供清晰、携带上下文的错误信息 (`LingoError`)，指出配置错误的来源和原因。
...
*   **架构模式：** 模块化与运行时元数据驱动。
*   **设计原理：**
    *   **`macros` Crate (过程宏库)：** 此 crate 负责在编译时处理用户定义的配置结构体。它通过 `#[derive(Config)]` 宏、结构体级 `#[config(...)]` 属性（如 `env_prefix`）和字段级 `#[lingo_opt(...)]` 属性，解析结构体定义和字段元数据。其核心职责包括：
        1.  **解析宏参数：** 从 `#[config(...)]` 中提取 `env_prefix`。
        2.  **解析字段属性：** 详细解析每个字段上的 `#[lingo_opt(...)]` 属性，提取如默认值表达式、描述、各种名称覆盖、`flatten` 标记、`skip` 标记以及 `clap` 特定属性等信息。
        3.  **行为版本控制：** 宏内部的生成逻辑保持一致，以确保稳定的行为。
        4.  **生成 `ClapArgs` 结构体：**
            *   动态创建一个内部的、临时的结构体定义（例如 `__{UserStructName}ClapArgs`），并为其派生 `clap::Parser`。
            *   根据 `#[lingo_opt(...)]` 中的 `description`, `default`, `name_clap_long/short`, `clap(...)` 等属性，为 `ClapArgs` 的字段生成相应的 `clap` 属性。
            *   特殊处理 `--config` 参数，用于指定配置文件路径。
        5.  **生成默认值函数：** 对于每个带有 `#[lingo_opt(default = <value_expr>)]` 的字段，生成一个私有的静态函数，该函数返回指定的默认值。例如，`fn __default_field_name() -> FieldType { <value_expr> }`。
            *   **安全增强：** 在此阶段，如果字段名暗示敏感信息（如 `password`, `secret` 等）且 `default` 属性被使用，则触发编译时警告。
        6.  **修改用户结构体定义 (用于 `serde`)：**
            *   为用户结构体的每个相关字段添加 `#[serde(default = "path::to::__default_field_name")]` 属性（如果该字段有 `lingo_opt(default)`）。
    ...
    *   **`core` Crate (运行时库)：** 此 crate 包含 `Lingo` 的所有运行时逻辑。它定义了元数据结构体，提供了用于路径解析、Provider 创建、配置加载和模板生成的模块。其核心职责包括：
        1.  **定义元数据结构：** 定义 `LingoAppMeta` (存储应用名、环境前缀、行为版本、解析深度)、`StructMeta` (存储结构体级元数据) 和 `FieldMeta` (存储字段级元数据) 等结构体。这些结构体用于在运行时表示和使用从编译时收集的信息。
        ...
        3.  **自定义 `figment` Provider 实现 (`providers` 模块)：**
            *   `LingoFileProvider`: 封装 `figment` 的文件加载，处理文件可选性（必需 vs. 可选），支持同步和异步文件读取，并应用可配置的解析深度限制。
            *   `LingoEnvProvider`: 实现从环境变量加载配置的逻辑。它会根据 `StructMeta` 和 `LingoAppMeta`（包括全局前缀、嵌套结构、`flatten` 规则、`name_env` 覆盖）动态构造预期的环境变量名，并将其值映射到 `figment` 理解的键路径。
            *   `LingoClapProvider`: 实现从 `clap` 解析结果加载配置的逻辑。它会特别处理布尔参数的来源（确保仅当用户显式提供时才覆盖低优先级源），并将其余命令行参数值提供给 `figment`。
        4.  **核心加载与编排逻辑 (`runtime` 模块)：** 包含 `load_config` (同步) 和 `load_config_async` (异步, 条件编译) 函数。这些函数是配置加载过程的总指挥，负责：实例化 `figment::Figment`；按正确的优先级顺序创建并添加所有 `Lingo*Provider` 实例；调用 `figment.extract()` 或 `figment.extract_async()` 将配置值反序列化到用户结构体中；以及处理和转换错误。
        ...
        6.  **错误类型定义 (`error` 模块)：** 定义 `LingoError` 枚举，这是一个全面的错误类型，用于报告配置加载过程中可能发生的各种问题，并携带详细的上下文信息。
        7.  **公共 API 定义 (`prelude` 模块与 `lib.rs`)：** 导出用户需要直接使用的类型和 trait，如 `Config` trait、`LingoError` 枚举、`TemplateFormat` 枚举以及 `Config` 派生宏本身。
...
项目根目录为 `lingo/`，采用 Cargo 工作区 (workspace) 结构。
...
│   └── basic_usage.rs      # 一个基本的 Lingo 使用示例
└── tests/                  # 集成测试和宏测试的根目录
    ├── integration/        # 集成测试
    │   └── priority_test.rs
    │   # ... 其他集成测试文件 ...
...
*   **`macros` Crate (位于 `lingo/macros/`)**
    *   **`macros/src/lib.rs`**:
        *   **主要组件**: `#[proc_macro_derive(Config, attributes(config, lingo_opt))]` 宏实现。
        *   **职责**:
            1.  **解析 `#[config(...)]` 结构体级属性**:
*   `env_prefix: String` (可选, 例如 `#[config(env_prefix = "MY_APP_")]`): 全局环境变量前缀。
            2.  **解析 `#[lingo_opt(...)]` 字段级属性**: 对每个字段，详细解析其上的 `#[lingo_opt(...)]` 属性，提取所有配置项（如 `default`, `description`, `name_config`, `name_env`, `name_clap_long`, `name_clap_short`, `flatten`, `skip`, `clap(...)`）。
            3.  **敏感默认值编译时警告**: 如果 `#[lingo_opt(default = "...")]` 被使用，且字段名（`rust_name`）包含如 "password", "secret", "token", "api_key", "passwd", "credentials" 等常见敏感词列表中的子串（不区分大小写），则使用 `proc_macro::Diagnostic` API (如果 Rust 版本支持稳定化，或通过 `proc_macro_error` crate) 发出一个编译时警告。
            4.  **生成 `__{UserStructName}ClapArgs` 结构体**:
                ...
                *   根据从 `#[lingo_opt(...)]` 收集到的信息，为 `__{UserStructName}ClapArgs` 的相应字段生成 `clap` 属性（如 `#[clap(long = "...", short = '...', help = "...", default_value_t = ..., action = ...)]` 等）。
            ...
            5.  **生成默认值函数**:
                *   对于每个在 `#[lingo_opt(default = "<expr_str>")]` 中提供了有效 Rust 表达式字符串 `<expr_str>` 的字段：
                    *   生成一个唯一的、私有的 (`#[doc(hidden)]`)、静态的函数，例如 `fn __lingo_default_{struct_name}_{field_name}() -> FieldType { <expr_str> }`。
                    *   `<expr_str>` 被直接用作此函数体内的代码。
                    *   `FieldType` 是该字段的实际 Rust 类型。
            6.  **修改用户结构体定义 (用于 `serde`)**:
                *   确保用户结构体派生了 `#[derive(serde::Deserialize)]`。如果用户已自行派生，则不重复添加。
                *   对于每个生成了默认值函数的字段，向该字段添加 `#[serde(default = "crate::path::to::__lingo_default_{struct_name}_{field_name}")]` 属性。路径必须是可从用户结构体定义处访问的。
                *   对于在 `#[lingo_opt(flatten)]` 中标记的字段，向该字段添加 `#[serde(flatten)]` 属性。
            7.  **生成元数据构建代码**:
                *   生成一个静态函数，例如 `fn __lingo_get_meta_{struct_name}() -> &'static crate::meta::StructMeta`。
                *   此函数负责在首次调用时（或作为静态初始化的一部分）构建并返回一个包含该结构体所有元数据（`StructMeta` 及其包含的 `FieldMeta` 列表）的静态引用。
                *   `FieldMeta` 的构建需要使用从 `#[lingo_opt(...)]` 解析出来的所有信息。
            8.  **实现 `Config` Trait**:
*   为用户结构体生成 `impl crate::Config for UserStructName { ... }`。
                *   **`load()` 方法实现**:
                    *   调用内部生成的 `__{UserStructName}ClapArgs::parse()` 获取 `clap_args_struct` 和 `clap_matches`。
                    *   构造 `crate::meta::LingoAppMeta` 实例。
                    *   调用 `__lingo_get_meta_{struct_name}()` 获取元数据。
...
*   **`core` Crate (位于 `lingo/core/`)**
    *   **`core/src/lib.rs`**:
        *   **职责**: Crate 入口点。声明所有公共模块 (`pub mod error;` 等)。定义并导出 `pub trait Config`。通过 `prelude` 模块选择性地重新导出最常用的公共 API。
    *   **`core/src/prelude.rs`**:
        *   **职责**: 提供一个方便的模块，用户可以通过 `use lingo::prelude::*;` 导入常用项。
        *   **导出内容**: `Config` trait, `LingoError` enum, `TemplateFormat` enum, `Config` 派生宏 (通过 `pub use macros::Config;`)。
    *   **`core/src/error.rs`**:
        *   **主要组件**: `pub enum LingoError { ... }` (使用 `#[derive(thiserror::Error, Debug)]`)。
        *   **职责**: 定义所有 `Lingo` 库可能产生的错误类型，确保每个错误变体都携带足够的上下文信息以便用户调试。详见第 14 节。
    *   **`core/src/meta.rs`**:
        *   **主要组件**:
            *   `pub struct LingoAppMeta { pub app_name: String, pub env_prefix: Option<String>, pub behavior_version: u32, pub max_parse_depth: u32 }`
    ...
    *   **`core/src/providers/file_provider.rs`**:
        *   **主要组件**: `pub(crate) struct LingoFileProvider { path: std::path::PathBuf, format: figment::providers::Format, is_required: bool, max_parse_depth: u32 }`
    ...
    *   **`core/src/providers/env_provider.rs`**:
        *   **主要组件**: `pub(crate) struct LingoEnvProvider<'a> { app_meta: &'a LingoAppMeta, struct_meta: &'a StructMeta }`
        *   **职责**: 实现 `figment::Provider` trait。
            *   `new(app_meta: &'a LingoAppMeta, struct_meta: &'a StructMeta) -> Self`
    ...
    *   **`core/src/providers/clap_provider.rs`**:
        *   **主要组件**: `pub(crate) struct LingoClapProvider<'a, 'm> { matches: &'m clap::ArgMatches, struct_meta: &'a StructMeta }`
        *   **职责**: 实现 `figment::Provider` trait。
    ...
    *   **`core/src/runtime.rs`**:
        *   **主要组件**:
            *   `pub(crate) fn load_config<T>(app_meta: LingoAppMeta, struct_meta: &'static StructMeta, clap_args_struct: GeneratedClapArgsStruct, clap_matches: clap::ArgMatches) -> Result<T, LingoError>`
            *   `#[cfg(feature = "async")] pub(crate) async fn load_config_async<T>(...) -> Result<T, LingoError>`
        *   **职责**: 编排整个配置加载过程。
            1.  初始化 `figment::Figment`。
            2.  调用 `paths::resolve_config_files()`。
            3.  按优先级顺序创建并添加 `LingoFileProvider` (系统级，用户级，命令行指定)。
            4.  创建并添加 `LingoEnvProvider`。
            5.  创建并添加 `LingoClapProvider`。
    ...
    *   **`core/src/template.rs`**:
        *   **主要组件**:
            ...
            *   `pub(crate) fn generate_template<T: Config>(struct_meta: &'static StructMeta, format: TemplateFormat) -> Result<String, LingoError>`
...
1.  **工作区与 Crate 结构搭建**:
    *   创建 `lingo/Cargo.toml` (工作区配置)。
    *   创建 `lingo/core/Cargo.toml` 和 `lingo/core/src/lib.rs`。
    *   创建 `lingo/macros/Cargo.toml` 和 `lingo/macros/src/lib.rs`。
    *   创建 `lingo/tests/` 目录结构 (`integration/`, `macros/pass/`, `macros/fail/`)。
2.  **`core` Crate - 核心类型定义**:
    *   `core/src/error.rs`: 定义 `LingoError` 枚举（包含所有变体和详细上下文）。
    *   `core/src/meta.rs`: 定义 `LingoAppMeta`, `StructMeta`, `FieldMeta` 的结构（包含所有字段如 `behavior_version`, `max_parse_depth`）。
...
*   **`#[proc_macro_derive(Config, attributes(config, lingo_opt))]`**
    *   **应用目标：** 用户定义的配置结构体。
    *   **结构体级属性 `#[config(...)]` (应用于结构体本身)：**
        ...
        *   `max_parse_depth = <integer_literal>` (可选, e.g., `64`, default e.g., `128`):
            *   建议的配置文件解析最大深度。传递给 `LingoFileProvider`。
            *   如果未提供，`LingoFileProvider` 将使用一个内部的合理默认值。
    *   **字段属性 `#[lingo_opt(...)]` (应用于结构体字段)：**
...
*   **`pub trait Config: Sized + serde::de::DeserializeOwned`**
    ...
    *   **方法：**
        ...
        ///
        /// # Errors
        /// Returns `LingoError` if any part of the loading process fails,
        /// such as file I/O errors, parsing errors, or missing required values
        /// without defaults.
        fn load() -> Result<Self, crate::error::LingoError>;
        ...
        /// Returns `LingoError` if any part of the loading process fails.
        #[cfg(feature = "async")]
        fn load_async() -> impl std::future::Future<Output = Result<Self, crate::error::LingoError>> + Send;
        ...
        ///
        /// # Errors
        /// Returns `LingoError` if template generation fails for any reason.
        fn generate_template(format: crate::template::TemplateFormat) -> Result<String, crate::error::LingoError>;
        ```

*   **`pub enum LingoError`**
    *   **定义位置：** `lingo/core/src/error.rs`
    *   **职责：** 统一的错误类型，携带详细上下文。
    *   **详细定义：** 见第 14 节 "错误处理 (`LingoError`) 详细定义"。

*   **`#[derive(Debug, Clone, Copy, PartialEq, Eq)] pub enum TemplateFormat`**
    *   **定义位置：** `lingo/core/src/template.rs` (或 `lib.rs` 并导出)
    *   **变体：**
        *   `Toml`
        *   `Json`
        *   `Ini`
...
2.  **宏生成的 `load` 方法执行**:
    a.  **初始化 `LingoAppMeta`**:
        ...
        *   构造 `core::meta::LingoAppMeta` 实例。
    b.  **解析命令行参数**:
        *   调用内部生成的 `__{UserStructName}ClapArgs::try_parse_from(std::env::args_os())`。
        *   获取 `clap_args_struct` 和 `clap_matches`。
    c.  **获取结构体元数据**:
        *   调用宏生成的静态函数 `__lingo_get_meta_{UserStructName}()`，该函数返回 `&'static core::meta::StructMeta`。
    d.  **调用核心加载逻辑**:
...
3.  **`core::runtime::load_config` 函数执行**:
    ...
    e.  **添加系统和用户配置文件 Providers**:
        *   遍历返回的路径列表。对每个 `ConfigFilePath`：
            *   创建 `core::providers::LingoFileProvider::new(path.path, format_enum, path.is_required, app_meta.max_parse_depth)`。
            *   调用 `figment = figment.merge(file_provider);`。
    f.  **添加命令行指定的配置文件 Provider**:
        *   从 `clap_args_struct.config` 获取路径。如果提供：
            *   创建 `core::providers::LingoFileProvider::new(path, format_enum, true, app_meta.max_parse_depth)`。
            *   `figment = figment.merge(specified_file_provider);`
    g.  **添加环境变量 Provider**:
        *   创建 `core::providers::LingoEnvProvider::new(&app_meta, struct_meta);`
        *   `figment = figment.merge(env_provider);`
    h.  **添加命令行参数 Provider**:
        *   创建 `core::providers::LingoClapProvider::new(&clap_matches, struct_meta);`
        *   `figment = figment.merge(clap_provider);`
    i.  **提取配置**: 调用 `figment.extract::<T>()`。
    j.  **返回结果**: `Ok(extracted_config)` 或 `Err(LingoError)`.
...
2.  **宏生成的 `load_async` 方法执行**:
    a.  与同步版本相同的 `LingoAppMeta` 初始化、`clap` 解析、`StructMeta` 获取。
...
3.  **`core::runtime::load_config_async` 函数执行**:
    ...
    e.  **添加系统和用户配置文件 Providers (异步感知)**:
        *   遍历路径列表。对每个 `ConfigFilePath`，创建 `LingoFileProvider` (传入 `max_parse_depth`)。
        *   调用 `figment = figment.merge(file_provider);`
    f.  **添加命令行指定的配置文件 Provider (异步感知)**: 同上。
    g.  **添加环境变量 Provider**: `LingoEnvProvider` 的 `data_async` 可以委托给同步 `data`。
    h.  **添加命令行参数 Provider**: `LingoClapProvider` 的 `data_async` 可以委托给同步 `data`。
...
2.  **宏生成的 `generate_template` 方法执行**:
    a.  **获取结构体元数据**: 调用宏生成的静态函数 `__lingo_get_meta_{UserStructName}()`。
...
3.  **`core::template::generate_template` 函数执行**:
    ...
    e.  **在 `build_template_recursive` 内部**:
        ...
        *   对于每个未被 `#[lingo_opt(skip)]` 的 `field_meta`:
    ...
    f.  返回构建好的 `String`，或 `LingoError::TemplateGeneration`。

**5.4. 环境变量名构造逻辑 (在 `LingoEnvProvider` 中)**
...
description = "Lingo (玲珑): A flexible and easy-to-use configuration library for Rust applications."
readme = "README.md"
...
# Proc Macro Crates (仅 macros 会直接依赖)
syn = { version = "2.0.100", features = ["full", "extra-traits"] }
quote = "1.0.40"
proc-macro2 = "1.0.95"
darling = "0.20.11"

# Lingo crates themselves (用于成员间依赖)
macros = { path = "macros", version = "0.2.0" }
...
description = "Core runtime library for Lingo, providing configuration loading and management."
...
authors.workspace = true
repository.workspace = true
description = "Procedural macros for the Lingo configuration library."
readme.workspace = true
keywords.workspace = true
categories.workspace = true
...
*   **命令行参数 Provider (`LingoClapProvider`) 行为：** 明确决定通过检查 `clap::ArgMatches::value_source()` 来仅处理用户显式提供的命令行参数（特别是对布尔值），以避免 `clap` 的内部默认值不期望地覆盖低优先级配置源。
...
*   **错误处理：** 使用 `thiserror` 定义统一的 `LingoError` 枚举，提供清晰的错误来源信息和丰富的上下文。
*   **模板生成中对必需字段的处理：** JSON 中用 `null`，TOML/INI 中用注释和类型提示。
*   **`default` 属性实现：** 通过生成静态默认值函数并使用 `#[serde(default = "...")]` 指向它，而不是直接嵌入表达式字符串。这允许更复杂的默认值表达式。
*   **嵌套结构体处理：** 子结构体需 `Deserialize`，可选 `Config` (如果需要其字段的 `config`/`lingo_opt` 特性)。命名规则和 `flatten` 行为已定义，以直观地映射配置。
...
*   **主动安全硬化措施：**
    *   **解析深度限制：** 通过内置默认策略和 `LingoFileProvider` 的实现，防御针对配置文件解析的资源耗尽攻击。
    *   **敏感默认值编译时警告：** `macros` crate 在编译时检测潜在的硬编码敏感默认值，并向开发者发出警告，提升安全意识。
*   **强化开发者体验 (`default` 表达式上下文文档)：** 通过详细文档和优化的宏错误提示，帮助用户正确使用 `#[lingo_opt(default = "...")]` 这一强大但易错的功能。
...
用户通过定义普通的 Rust 结构体来指定其应用程序的配置。这些结构体必须满足以下条件才能与 `Lingo` 一起使用：

*   **必须派生 `#[derive(Config)]`**: 这是启用 `Lingo` 功能的入口点。
...
*   **字段属性**: 可以使用 `#[lingo_opt(...)]` 属性来定制字段的行为。
...
struct AppConfig {
    #[lingo_opt(description = "Server host address")]
    host: String,

    #[lingo_opt(default = "8080", description = "Server port number")]
    port: u16,

    #[lingo_opt(description = "Enable feature X", name_clap_long = "enable-x")]
    feature_x_enabled: bool,

    #[lingo_opt(skip, description = "This field is ignored by Lingo")]
    #[allow(dead_code)]
    temp_data: String,

    database: DatabaseConfig,

    #[lingo_opt(description = "Optional API key for external service")]
...
struct DatabaseConfig {
    #[lingo_opt(default = r#""postgres://user:pass@localhost/mydb".to_string()"#)]
    url: String,
    #[lingo_opt(default = "5")]
    pool_size: u32,
}
...
struct WorkerConfig {
    #[lingo_opt(default = "4")]
    threads: usize,
    #[lingo_opt(description = "Queue name for workers")]
    queue_name: String,
}
```
...
*   **`pub struct FieldMeta`**: (每个实例代表用户结构体中的一个字段)
    ...
    *   `pub config_name_override: Option<&'static str>`: 来自 `#[lingo_opt(name_config = "...")]` 的值。
    *   `pub env_name_override: Option<&'static str>`: 来自 `#[lingo_opt(name_env = "...")]` 的值。
    *   `pub clap_long_override: Option<&'static str>`: 来自 `#[lingo_opt(name_clap_long = "...")]` 的值。
    *   `pub clap_short_override: Option<char>`: 来自 `#[lingo_opt(name_clap_short = "...")]` 的值。
    *   `pub description: Option<&'static str>`: 来自 `#[lingo_opt(description = "...")]`。
    ...
    *   `pub is_flatten: bool`: 标记该字段是否有 `#[lingo_opt(flatten)]`。
    *   `pub is_skipped: bool`: 标记该字段是否有 `#[lingo_opt(skip)]`。
    *   `pub clap_direct_attrs_meta: Option<ClapAttrsMeta>`: 结构化表示来自 `#[lingo_opt(clap(...))]` 的原生 `clap` 属性。`ClapAttrsMeta` 会包含如 `long: Option<String>`, `short: Option<char>`, `help: Option<String>`, `action_code: Option<TokenStream>`, `value_parser_code: Option<TokenStream>` 等。
...
*   通过 `#[config(...)]` 和 `#[lingo_opt(...)]` 属性以声明方式定制行为。
*   提供简洁的 API (`load()`, `load_async()`, `generate_template()`).
    *   清晰的错误消息 (`LingoError` 携带上下文)。
    *   自动处理多源加载和优先级。
    *   **`default` 表达式上下文文档：** 在用户文档和 `#[lingo_opt(default = "...")]` 的 API 文档中详细说明表达式的编译上下文、可见性规则和正确使用示例。
...
**10.3. 可定制性**
*   **实现：**
    *   `#[lingo_opt(...)]` 提供对字段命名、默认值、描述、`clap` 参数等的细粒度控制。
...
*   **实现：**
    ...
    *   **解析深度限制：** `LingoFileProvider` 在调用底层文件解析器时，应用内置的解析深度限制（可在内部通过常量或配置项控制），以防止因恶意构造的深嵌套配置文件导致的资源耗尽。
    *   **敏感默认值编译时警告：** `macros` crate 在编译时扫描 `#[lingo_opt(default = "...")]` 的字段名，如果匹配常见的敏感词列表，则发出编译警告，提醒开发者避免在源代码中硬编码敏感默认值。
...
*   **行为：**
    *   如果用户未指定 `version`，宏默认采用当前 `Lingo` 库版本所定义的最新行为。
    *   如果用户指定了 `version = N`，则宏（即使是更新版本的 `Lingo` 库中的宏）会生成与 `Lingo` 行为版本 `N` 相兼容的代码。
...
`#[lingo_opt(...)]` 是一个应用于结构体字段的属性，用于定制该字段在配置加载、命令行参数生成和模板生成等方面的行为。

**11.1. `default = <value_expr_string>`**
*   **类型：** `<value_expr_string>` 是一个字符串字面量，其内容必须是一个有效的 Rust 表达式。
*   **行为：**
    1.  `macros` crate 解析此字符串表达式。
    2.  宏生成一个私有的静态函数，例如 `fn __lingo_default_{struct_name}_{field_name}() -> FieldType { <value_expr_string 的内容作为代码> }`。此函数的返回类型 `FieldType` 必须与应用此属性的字段的类型相匹配。
...
*   **示例：**
    *   `#[lingo_opt(default = "8080")] port: u16,`
    *   `#[lingo_opt(default = r#""localhost".to_string()"#)] host: String,`
    *   `#[lingo_opt(default = "std::path::PathBuf::from(\"/var/log/app.log\")")] log_path: std::path::PathBuf,`
    *   `#[lingo_opt(default = "vec![1, 2, 3]")] numbers: Vec<i32>,`
...
*   **示例：** `#[lingo_opt(description = "The network port to listen on.")] port: u16,`
...
*   **行为：**
    ...
    3.  `LingoEnvProvider` 和 `LingoClapProvider` 在构造 `figment` 键路径时，如果需要从配置文件键名派生，会使用此覆盖值。
    4.  `generate_template()` 会使用此名称作为模板中的键。
*   **示例：** `#[lingo_opt(name_config = "serverPort")] port: u16,`

**11.4. `name_env = "<string_literal>"`**
*   **类型：** `<string_literal>` 是一个字符串字面量，通常应符合环境变量的命名约定（大写，下划线分隔）。
*   **行为：**
    1.  **完全覆盖**此字段对应的环境变量名。
    2.  如果提供此属性，`LingoEnvProvider` 将直接使用此字符串作为环境变量名来查找，而**不会**应用全局 `env_prefix` 或基于结构体路径和字段名的自动命名规则。
    3.  如果不提供，环境变量名将根据全局 `env_prefix` (如果存在)、结构体路径 (考虑嵌套和 `flatten`) 和字段的 Rust 名称 (转换为大写蛇形) 自动生成。
*   **示例：** `#[lingo_opt(name_env = "SERVICE_PORT")] port: u16,`
...
*   **行为：**
    ...
    3.  此属性优先于 `clap` 的自动命名，但会被 `#[lingo_opt(clap(long = "..."))]` 中的显式设置覆盖。
*   **示例：** `#[lingo_opt(name_clap_long = "server-port")] port: u16,`
...
*   **行为：**
    ...
    3.  此属性会被 `#[lingo_opt(clap(short = ...))]` 中的显式设置覆盖。
*   **示例：** `#[lingo_opt(name_clap_short = 'p')] port: u16,`
...
*   **类型：** 标记属性。
*   **行为：**
    1.  此字段将被 `Lingo` 完全忽略（不参与配置加载、命令行参数、模板生成）。
    2.  `Lingo` 的 `skip` 不会自动添加 `#[serde(skip)]`。
    3.  **元数据影响：** `FieldMeta.is_skipped` 设为 `true`。
*   **示例：** `#[lingo_opt(skip)] transient_state: MyInternalType,`
...
*   **行为：**
    ...
    3.  **优先级与合并：** `clap(...)` 内指定的属性通常具有最高优先级，会覆盖由其他 `lingo_opt` 属性（如 `description`, `default`, `name_clap_long/short`）推断出的 `clap` 属性。
*   **示例：**
    ```rust
    #[lingo_opt(
...
`Lingo` 对嵌套结构体的支持是其核心功能之一，旨在让复杂的配置结构易于管理。
...
*   **定义：** 用户可以在一个派生了 `#[derive(Config)]` 的结构体中，包含另一个结构体作为字段。
    ...
    // #[derive(Config)] // 可选：若 ServerConfig 的字段需要 lingo_opt 特性
    struct ServerConfig {
        host: String,
        port: u16,
    }
...
*   **对嵌套结构体的要求：**
    ...
    2.  **`Config` (可选但推荐用于其字段特性)：** 如果嵌套结构体的字段本身需要使用 `#[lingo_opt(...)]` 属性（例如，为其字段指定 `default` 值），那么该嵌套结构体**也应该**派生 `#[derive(Config)]`。
*   **配置加载行为 (无 `flatten`)：**
...
*   **使用 `#[lingo_opt(flatten)]` 处理嵌套结构体：**
    *   当在父结构体的嵌套结构体字段上使用 `#[lingo_opt(flatten)]` 时：
...
*   **定义：**
    ...
    #[derive(Config, Debug)]
    struct Config {
        #[lingo_opt(description = "An optional API key")]
        api_key: Option<String>,
        #[lingo_opt(default = "Some(3u8)")]
        retries: Option<u8>,
    }
    ```
*   **行为：**
    1.  **`serde::Deserialize`：** 如果配置源中不存在该键，字段为 `None`。
    2.  **`Lingo` 加载逻辑：** 如果 `Option<T>` 字段在所有配置源中都没有值，且无 `default`，则加载后为 `None`。这不是错误。
    3.  **默认值 (`#[lingo_opt(default = ...)]`) 与 `Option<T>`：** 可以为 `Option<T>` 字段指定 `Some(...)` 或 `None` 作为默认值。
...
/// The unified error type for all operations within the Lingo library.
#[derive(Error, Debug)]
pub enum LingoError {
...
*   每个可能失败的函数返回 `Result<T, LingoError>`。
*   底层错误被转换为 `LingoError` 变体，保留源错误信息。
*   错误消息力求清晰和可操作。
...
为确保 `Lingo` 库的质量、健壮性和正确性，将采用以下多层次的测试策略：
...
*   **具体内容：**
    ...
    *   **`providers/*.rs`**: 对每个 Provider 进行独立测试。`LingoEnvProvider` 测试需设置和清理环境变量。`LingoFileProvider` 测试文件读写、解析、错误处理和深度限制。
    *   **`template.rs`**: 对每种格式的模板生成进行快照测试 (使用 `insta` crate)。
    *   **`runtime.rs`**: 测试内部辅助函数，`app_name` 解析逻辑。

**15.2. 宏测试 (`macros` Crate)**
*   **位置：** `lingo/macros/tests/macros/` (或 `lingo/macros/tests/`)。
*   **工具：** **强制要求使用 `trybuild` crate。**
*   **目标：** 验证 `#[derive(Config)]` 宏在各种输入下的行为。
*   **具体内容：**
    *   **`pass/` 子目录：** 包含所有应该成功编译的宏用法示例（简单、嵌套、`flatten`、所有 `lingo` 和 `lingo_opt` 属性等）。
    *   **`fail/` 子目录：** 包含所有应该导致编译失败的用法，并验证编译器错误信息（例如，属性参数类型错误、`default` 表达式语法错误等）。测试敏感默认值警告是否按预期触发。

**15.3. 集成测试 (项目根 `lingo/tests/` 目录)**
*   **位置：** `lingo/tests/integration/` (或直接在 `lingo/tests/`)。
...
*   **具体内容：**
    ...
    *   **错误处理测试**: 故意制造错误条件，断言返回的 `LingoError` 类型和内容符合预期。
    *   **安全特性测试**: 测试解析深度限制。

**15.4. 文档测试**
*   **位置：** `README.md`，`lingo/core/src/lib.rs`，`lingo/core/src/prelude.rs` 及其他公共 API 文档注释。
*   **工具：** `cargo test --doc`。
